    ...
    for n in range(len(TEST_SET)):
        ...
        for i in range(padding_h // stride):
            for j in range(padding_w // stride):
                ...
                mask_whole[i * stride:i * stride + image_size, j * stride:j * stride + image_size, :] = pred[0]
                print("i:%d" % i, "j:%d" % j)
        result[0:h, 0:w, :] = mask_whole[0:h, 0:w, :]
        result_bgr = result[..., ::-1]
        cv2.imwrite("./Result/" + path.split(".")[0] + "_label.tif", result_bgr)
        print("./Result/" + path.split(".")[0] + "_label.tif  saved")
...
with tf.Session(config=config) as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        saver.restore(sess, checkpoint_path + "model.ckpt-2")
