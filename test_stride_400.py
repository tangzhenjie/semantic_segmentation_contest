...
def predict(TEST_SET, sess, prediction, imgs_batch):
    for n in range(len(TEST_SET)):
        ...
        image = cv2.imread('/2T/tzj/semantic_segmentation_contest/DatasetOrigin/val/' + path, cv2.IMREAD_UNCHANGED)
        ...
        padding_img[50:50 + h, 50:50 + w, :] = image[:, :, 0:3]
        ...
        for i in range(padding_h // stride):
            for j in range(padding_w // stride):
                ...
                mask_whole[50 + i * stride: 50 + i * stride + stride, 50 + j * stride: 50 + j * stride + stride, :] = pred[0][50:450, 50:450, :]
                print("i:%d" % i, "j:%d" % j)
        result[0:h, 0:w, :] = mask_whole[50:50 + h, 50:50 + w, :]
        result_bgr = result[..., ::-1]
        cv2.imwrite("./Result_stride_400/" + path.split(".")[0] + "_label.tif", result_bgr)
        print("./Result_stride_400/" + path.split(".")[0] + "_label.tif  saved")

#cv2.imwrite('./predict/pre' + str(n + 1) + '.png', mask_whole[0:h, 0:w])
# 获取全部测试集图片
TEST_SET = os.listdir('/2T/tzj/semantic_segmentation_contest/DatasetOrigin/val')
...
with tf.Session(config=config) as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        saver.restore(sess, checkpoint_path + "model.ckpt-10")
