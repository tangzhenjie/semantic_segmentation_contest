...
batch_size = 4
summary_path = "./summary_deeplabv3plus_4/"
checkpoint_path_first = "./checkpoint_deeplabv3plus/" # 训练好的三通道deeplabv3+
checkpoint_path = "./checkpoint_deeplabv3plus_4/"
EPOCHS = 30
...
# poly learn_rate
parser.add_argument('--initial_learning_rate', type=float, default=4e-3,
...
parser.add_argument('--max_iter', type=int, default=37500,
...
def main():
    ...
    loss, train_op, metrics = tools_deeplabv3plus.get_loss_pre_metrics(x, y, is_train, batch_size, args)
    ...
    # 首次运行从deeplabv3+中获取权重需要剔除logits层
    exclude = [args.resnet_model + '/conv1', 'global_step']
    ...
    # 获得变量值
    checkpoint_path_restored = os.path.join(checkpoint_path_first, "model.ckpt-58")
    ...
    with tf.Session(config=config) as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            saver_first.restore(sess, checkpoint_path_first+"model.ckpt-58")
        sess.graph.finalize()
        ...
        for epoch in range(EPOCHS):
            ...
            for step in range((epoch * train_batches_of_epoch), ((epoch + 1) * train_batches_of_epoch)):
                ...
                if (step + 1) % 625 == 0:
                    loss_value, acc, m_iou, con_matrix, merge = sess.run(
                        [loss, accuracy, mean_iou, confusion_matrix, summary_op],
                        feed_dict={x: img_batch, y: label_batch, is_train: True})
                    kappa = tools_deeplabv3plus.kappa(con_matrix)
