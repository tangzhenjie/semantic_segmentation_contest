...
batch_size = 3
summary_path = "./summary_position/"
checkpoint_path_first = "./first_checkpoint/"
checkpoint_path = "./checkpoint_position/"
EPOCHS = 50
...
def main():
    ...
    x = tf.placeholder(dtype=tf.float32, shape=[None, 1000, 1000, 3], name="image_batch")
    ...
    loss, train_op, metrics = tools_deeplabv3_DA.get_loss_pre_metrics(x, y, is_train, batch_size, args)
    ...
    exclude = ['global_step', 'DeepLab_v3/ASPP_layer/image_level_conv_1x1', 'DeepLab_v3/ASPP_layer/reduce_chanel', 'DeepLab_v3/ASPP_layer/position_module']
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(EPOCHS):
            ...
            for step in range((epoch * train_batches_of_epoch), ((epoch + 1) * train_batches_of_epoch)):
                ...
                if (step + 1) % 833 == 0:
                    kappa = tools_deeplabv3_DA.kappa(con_matrix)
